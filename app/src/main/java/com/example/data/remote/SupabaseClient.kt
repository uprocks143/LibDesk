...
object SupabaseClient {
    ...
    var apiKey: String = DEFAULT_PUBLISHABLE_KEY
        private set
    ...
    suspend fun upsertRecords(tableName: String, jsonArray: JSONArray): Pair<Boolean, String> = withContext(Dispatchers.IO) {
        try {
            ...
            val request = Request.Builder()
                .url("$projectUrl/rest/v1/$tableName")
                .addHeader("apikey", apiKey)
                .addHeader("Authorization", "Bearer ${currentAuthToken ?: apiKey}")
            ...
            httpClient.newCall(request).execute().use { response ->
                ...
                if (response.isSuccessful || response.code == 201 || response.code == 200) {
                    Pair(true, "Synced ${jsonArray.length()} records to $tableName")
                } else {
                    Log.e(TAG, "Upsert to $tableName failed (${response.code}): $respBody")
                    Pair(false, "Table '$tableName' error (${response.code}): ${respBody.take(120)}")
                }
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error upserting to $tableName", e)
            Pair(false, "Failed to sync $tableName: ${e.message}")
        }
    }
    ...
    suspend fun fetchRecords(tableName: String, libraryId: String? = null): Pair<Boolean, JSONArray?> = withContext(Dispatchers.IO) {
        try {
            var url = "$projectUrl/rest/v1/$tableName?select=*"
...
-- Drop legacy functions & triggers if existing
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP FUNCTION IF EXISTS public.elevate_user_role();
DROP FUNCTION IF EXISTS auth.get_app_role();
...
-- =========================================================================
-- 3. ENABLE SUPABASE REALTIME PUBLICATION FOR LIVE SYNC
-- =========================================================================
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_publication WHERE pubname = 'supabase_realtime') THEN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.students, public.seats, public.attendance, public.payments, public.notices, public.feedback_complaints, public.physical_books, public.digital_materials;
  END IF;
EXCEPTION WHEN OTHERS THEN
  -- Realtime publication already contains these tables or is managed by dashboard
  NULL;
END $$;
        """.trimIndent()
    }
