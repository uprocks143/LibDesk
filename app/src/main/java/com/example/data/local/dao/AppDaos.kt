...
@Dao
interface NoticeDao {
    @Query("SELECT * FROM notices WHERE (libraryId = :libraryId OR libraryId = 'GLOBAL' OR libraryId = 'ALL') AND isActive = 1 ORDER BY date DESC")
    fun getActiveNoticesByLibrary(libraryId: String): Flow<List<NoticeEntity>>

    @Query("SELECT * FROM notices WHERE (libraryId = :libraryId OR libraryId = 'GLOBAL' OR libraryId = 'ALL') ORDER BY date DESC")
